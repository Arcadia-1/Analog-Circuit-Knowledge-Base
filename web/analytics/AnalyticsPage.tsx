...
import { landPathsForWorld, WORLD_BOUNDS, type FeatureCollection } from "./worldMap";
...
const MAP_H = Math.round(
  MAP_W * ((WORLD_BOUNDS.north - WORLD_BOUNDS.south) / (WORLD_BOUNDS.east - WORLD_BOUNDS.west)),
);
const LAND_PATHS = landPathsForWorld(land as unknown as FeatureCollection, MAP_W, MAP_H);
...
export default function AnalyticsPage() {
  ...
  useEffect(() => {
    ...
    const url = key ? `/api/analytics?key=${encodeURIComponent(key)}` : "/api/analytics";
    void fetch(url, {
  ...
  const rangeDays = from && to ? dayCount(from < to ? from : to, from < to ? to : from) : 90;
        ...
        <div className="analytics-container">
          <header className="analytics-head">
            ...
            <p className="analytics-sub">
              First-party counts · UTC days · updated <time dateTime={summary?.generatedAt}>{generated}</time>
            </p>
          </header>

          {error && (
            <p className="analytics-error">
              Could not load analytics. If a dashboard key is set, append <code>?key=…</code> to the URL.
            </p>
          )}
          ...
          <section className="analytics-section" aria-labelledby="analytics-map-h">
            <SectionHead
              title="Origins"
              id="analytics-map-h"
              aside="Map"
          ...
          <section className="analytics-section" aria-labelledby="analytics-chart-h">
            <SectionHead
              title="Daily traffic"
              ...
              meta={(
                <>
                  <ul className="analytics-legend" aria-hidden="true">
                    <li><span className="analytics-swatch analytics-swatch--pv" />Views</li>
                    <li><span className="analytics-swatch analytics-swatch--uv" />Visitors</li>
                  </ul>
            ...
            />
            <form className="analytics-range" onSubmit={(event) => event.preventDefault()}>
              ...
              <button type="button" className="analytics-range-reset" onClick={resetRange}>
                Last 90 days
              </button>
          ...
          <div className="analytics-cols">
            <section className="analytics-section" aria-labelledby="analytics-countries-h">
              <SectionHead title="ISO 3166 Code" id="analytics-countries-h" aside="Top" />
              <BreakdownTable
                heading="Region"
                rows={summary?.countries ?? []}
                total={summary?.breakdownTotals.countries ?? { pv: 0, uv: 0 }}
                label={(row) => countryName((row as Summary["countries"][number]).code)}
                loading={!summary && !error}
                unavailable={error}
              />
            </section>
            <section className="analytics-section" aria-labelledby="analytics-sources-h">
              <SectionHead title="Sources" id="analytics-sources-h" aside="Top" />
              <BreakdownTable
                heading="Source"
                rows={summary?.sources ?? []}
                total={summary?.breakdownTotals.sources ?? { pv: 0, uv: 0 }}
                label={(row) => sourceName((row as Summary["sources"][number]).source)}
                loading={!summary && !error}
                unavailable={error}
              />
            </section>
  ...
  return (
    <div className="analytics-section-head">
      ...
      <div className="analytics-section-head-meta">
        {meta}
        {aside && <span className="analytics-aside" aria-hidden="true">{aside}</span>}
      </div>
    </div>
  );
}
...
function DailyChart({ days }: { days: DayRow[] }) {
  ...
  if (!days.length) {
    return (
      ...
      >
        <text className="analytics-chart-tick" x={width / 2} y={height / 2} textAnchor="middle">
  ...
  const ticks = days.length === 1
    ...
    : days.length === 2
      ...
      : [
          { index: 0, x: 0, anchor: "start" as const },
          { index: Math.floor(days.length / 2), x: width / 2, anchor: "middle" as const },
  ...
  return (
    ...
    >
      ...
      <line
        ...
        y2={height - padBottom}
      />
      <text className="analytics-chart-tick" x="0" y={padTop - 2}>{fmt.format(max)}</text>
      {ticks.map((tick) => (
        <text
  ...
  return (
    <div className="analytics-map-frame">
      ...
      >
        ...
        <rect className="analytics-map-ocean" x="0" y="0" width={MAP_W} height={MAP_H} />
        <g className="analytics-map-land" aria-hidden="true">
          {LAND_PATHS.map((path, index) => <path d={path} key={index} />)}
        </g>
          ...
          {points.map((point) => {
            ...
            return (
              <g key={`${point.lat},${point.lng}`}>
                <circle className="analytics-heat-blob" cx={cx} cy={cy} r={radius}>
                  <title>{title}</title>
                </circle>
                <circle
  ...
  return (
    <div className="analytics-breakdown">
        ...
        <table className="analytics-table" id={tableId}>
          <thead>
            <tr>
              ...
              <th scope="col" className="num">Unique Visitors</th>
              <th scope="col" className="num">Page Views</th>
            </tr>
          </thead>
          <tbody>
            {loading || unavailable || rows.length === 0 ? (
              <tr>
                <td colSpan={3} className="analytics-empty">
                  {loading ? "Loading…" : unavailable ? "Unavailable." : "No data yet."}
                </td>
              </tr>
      ...
      {canExpand && (
        ...
        >
          <span>{expanded ? `Show top ${DEFAULT_BREAKDOWN_ROWS}` : `Show all ${rows.length}`}</span>
...
function MetricCell({ value, total }: { value: number; total: number }) {
  ...
  return (
    <td className="num analytics-metric">
      ...
      <span className="analytics-metric-distribution">{percentage.toFixed(1)}%</span>
    </td>
  );
}

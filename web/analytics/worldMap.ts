...
function ringToPath(ring: Ring, width: number, height: number): string {
  ...
  for (let index = 0; index < ring.length; index++) {
    ...
    const x = ((lng - WORLD_BOUNDS.west) / (WORLD_BOUNDS.east - WORLD_BOUNDS.west)) * width;
    const y = ((WORLD_BOUNDS.north - lat) / (WORLD_BOUNDS.north - WORLD_BOUNDS.south)) * height;
    path += `${index === 0 ? "M" : "L"}${x.toFixed(2)} ${y.toFixed(2)}`;
  }
  return `${path}Z`;
}

/** Build the same coastline-only equirectangular world used by tokenzhang.com. */

...
<main class="page">
  <header class="top">
    ...
    <h1>How long should the record be?</h1>
    ...
    <Notes>
      ...
      <p><b>That separates SNDR from SFDR.</b> SNDR adds every non-signal bin, so a longer record should converge to the same SNDR and ENOB. SFDR looks at the single tallest non-signal line. It rises while that line is a random noise bin, then stops when a real harmonic becomes tallest. Record length improves visibility, not converter linearity.</p>
      <p><b>How to choose <var>N</var>.</b> Start with the bin spacing you need: <var>N</var> ≥ <var>f</var><sub>s</sub>/Δ<var>f</var><sub>required</sub>. The window still decides how many adjacent bins one tone occupies. Then make the record long enough for the spur you care about to stand above the tallest noise bin. Report <var>N</var>, <var>f</var><sub>s</sub> and the window with any SFDR number.</p>
      <p><b>The experiment.</b> A {BITS}-bit converter runs at {FS / 1e6} MS/s with a coherent tone, a third harmonic at the level you set, and enough white noise to reach the ENOB you set. Each length gets {RUNS} fresh-noise captures; the shaded bands show their full range. Run-to-run uncertainty is treated separately in <a href="/adc/measurement-repeatability/">Measurement repeatability</a>.</p>

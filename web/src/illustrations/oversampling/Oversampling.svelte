...
<main class="page">
  <header class="top">
    ...
    <Notes>
      ...
      <p><b>Noise shaping.</b> The model quantises, takes the error, and passes it through NTF(<var>z</var>) = (1 − <var>z</var><sup>−1</sup>)<sup><var>L</var></sup> before adding it back: zero at DC, rising as (2 sin π<var>f</var>)<sup><var>L</var></sup>, a straight line of 20<var>L</var> dB a decade on the log axis. The total noise gets worse, two, six and twenty times for <var>L</var> = 1, 2, 3, but what stays in the band collapses, and each doubling of OSR now buys 6<var>L</var> + 3 dB.</p>
      <p><b>Two ways to count it.</b> <code>perfosr</code> fits the sine, windows the residual and adds up its spectrum to <var>f</var><sub>s</sub>/2·OSR, one OSR after another. <code>ntfperf</code> integrates |NTF|² over the same band on a million-point grid; the dashed lines add that to white quantisation noise of LSB²/12. They agree while the error behaves like noise. With few bits a clean tone's error is harmonics of the tone rather than noise, and the measured line strays from the dashed one, most at high OSR where only a few of those harmonics are left in the band.</p>
      ...
      <p><b>A model, not a loop.</b> <code>apply_noise_shaping</code> adds the shaped error to the input, the way a modulator's output looks, without a quantiser inside a feedback loop. It never overloads, where a real third-order modulator with a two-bit quantiser would. Its filter also starts from rest, and that first step is spread across the whole spectrum: it is the flat floor at the far left, and what finally stops the highest OSRs from gaining more.</p>
    </Notes>
  </header>
  ...
  <section class="controls" aria-label="Oversampling ratio and quantiser">
    ...
    <span class="about">{N} samples at 100 MHz · a {freqText(TONE.fin)} tone of 0.4 V on ±0.5 V</span>
  </section>

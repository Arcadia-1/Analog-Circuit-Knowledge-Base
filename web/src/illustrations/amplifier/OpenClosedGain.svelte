...
<style>
  ...
  .amplifier-page { width: 100%; height: 100%; min-height: 0; max-width: 1540px; padding: 12px 24px 9px; grid-template-rows: minmax(0, 1fr); }
  .setup { display: grid; gap: 12px; padding-bottom: 2px; }
  ...
  .design-controls { display: grid; gap: 8px; min-width: 0; }
  .constraint-row { display: grid; gap: 5px; }
  .constraint-row > .label { font-size: 10px; }
  .bandwidth-control { padding: 9px 12px; border-radius: 5px; background: var(--chip); }
  .bandwidth-control :global(.range) { display: grid; grid-template-columns: minmax(0, 1fr) 11ch; gap: 4px 10px; width: 100%; }
  .bandwidth-control :global(input) { grid-column: 1 / -1; width: 100%; min-width: 0; }
  ...
  @media (min-width: 821px) and (max-height: 740px) {
    ...
    .design-controls { gap: 4px; }
    .constraint-row { grid-template-columns: auto minmax(0, 1fr); align-items: center; }
    .bandwidth-control { padding: 5px 8px; }
    .bandwidth-control :global(.range) { grid-template-columns: 126px minmax(40px, 1fr) 8ch; gap: 8px; }
    .bandwidth-control :global(input) { grid-column: auto; }

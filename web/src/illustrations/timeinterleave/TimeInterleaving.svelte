<script lang="ts">
  ...
  const worst = $derived(r.tones.reduce((a, b) => (b.dbc > a.dbc ? b : a)));
...
<main class="page">
  ...
  <section class="compare">
    <div class="chart wide">
      ...
      <SpurSpectrum spectrum={r.out} ghost={method === 'off' ? null : r.raw} tones={r.tones} {bits} hover={hoverBin} onhover={(b) => (hoverBin = b)} label="Spectrum of the interleaved output with the predicted spurs" />
    </div>
    ...
    <div class="chart">
      <div class="cap">
        ...
        <span>largest spur <b>{nf(worst.dbc, 1)} dBc</b>, {worst.kind === 'offset' ? 'offset' : 'image'} k = {worst.ks.join(', ')}</span>
      </div>
